...
int main(void) {
    ...
    // Render texture initialization, used to hold the rendering result so we
    ...
    Color colors[10] = {0};
    for (int i = 0; i < 10; i++)
        colors[i] = (Color){GetRandomValue(100, 250), GetRandomValue(50, 150),
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        // Draw
        //----------------------------------------------------------------------------------
        // Draw everything in the render texture, note this will not be rendered
        // on screen, yet
        BeginTextureMode(target);
        ClearBackground(RAYWHITE); // Clear render texture background color

        for (int i = 0; i < 10; i++)
            DrawRectangle(0, (gameScreenHeight / 10) * i, gameScreenWidth,
